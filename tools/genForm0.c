...
#include "daisybox/include/config.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <stdbool.h>
...
#include "daisybox/include/bfd.h"
#include "daisybox/include/bfdlink.h"
...
static asection *readSection(bfd *abfd, FILE *inputFile, char *sectionName) {
    ...
    if (section->contents == NULL) {
        log_fatal("Can't allocate memory for section: %s", sectionName);
        perror("");
        return NULL;
    }
...
/*
 * This is where things will start to get a little bit messy..
 *
 * This function compute an approximated size of a UVMO file by
 * Taking the sections sizes of an input ELF and even, calculating
 * a possible mostly accurated size for RELA
 *
 */
int computeUvmoSize(const char *path) {
    ...
    int relaArraySize = 0;
    int sectionsSize = 0;
    asection *text = readSection(abfd, fp, ".text");
    asection *rodata = readSection(abfd, fp, ".rodata");
    asection *data = readSection(abfd, fp, ".data");
...
void startTableEntries(const int count, cJSON *files) {
    cJSON *file;
